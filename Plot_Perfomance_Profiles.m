...
for s = 1:ns
    for p = 1:np
        ...
        if (isempty(val)) || (isnan(val)&&~strcmp(A{p+1,s},'')) ||val == 0
            T(p,s) = NaN;
        else
            T(p,s) = val;
        end
...
for s = 1:ns
    ...
    if (logplot)
        hl(s) = semilogx(xs,ys,strcat(char(lines(sl)),markers(sm)),'MarkerFaceColor',rgb(char(colors(sc))),'MarkerSize',6);
    else
        hl(s) = plot(xs,ys,strcat(char(lines(sl)),markers(sm)),'MarkerFaceColor',rgb(char(colors(sc))),'MarkerSize',6);
    end
    hold on;
end
