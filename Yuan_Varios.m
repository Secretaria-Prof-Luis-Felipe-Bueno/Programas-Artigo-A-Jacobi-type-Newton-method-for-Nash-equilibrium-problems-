...
switch mystr
    case 'vacina'
     ...
     %Derivadas
     gl_x = @(s)  -(a*s2 - b1*ones(n,1)); %primeira derivada de u1 em relação a x
     gll_x = @(s) 0; %segunda derivada de u1 em relação a x

     gl_y = @(s) (a*s1 - b2*ones(m,1)); %primeira derivada de u2 em relação a y
    ...
    case 'problemayuan'
     ...
     %Derivadas     
     gl_x = @(s)  (s2 - b1*ones(n,1)); %primeira derivada de u1 em relação a x
     gll_x = @(s) zeros(n,n); %segunda derivada de u1 em relação a x

     gl_y = @(s) (s1 + b2*ones(m,1)); %primeira derivada de u2 em relação a y
    ...
    case 'cubico'
     ...
     %Derivadas
     gl_x = @(s)  (s1.^2).*(s2.^2)+s1; %primeira derivada de u1 em relação a x
     gll_x = @(s) eye(n)+2*s1*(s2.^2)'; %segunda derivada de u1 em relação a x

     gl_y = @(s) (s1.^2).*(s2.^2)+s2; %primeira derivada de u2 em relação a y
     gll_y = @(s) eye(m)+2*s2*(s1.^2)'; %segunda derivada de u2 em relação a y     
     ...
     %Derivadas
     gl_x = @(s)  2*(s1-a1); %primeira derivada de u1 em relação a x
     gll_x = @(s) 2*eye(n); %segunda derivada de u1 em relação a x

     gl_y = @(s) -2*(s2-b2); %primeira derivada de u2 em relação a y
     ...
     %Derivadas
     gl_x = @(s)  s1-b1; %primeira derivada de u1 em relação a x
     gll_x = @(s) eye(n); %segunda derivada de u1 em relação a x

     gl_y = @(s) s2-b2; %primeira derivada de u2 em relação a y
    ...
    case 'quartico'
     ...
     %Derivadas
     gl_x = @(s)  (s1-b1).^3 - s1.*(s2.^2); %primeira derivada de u1 em relação a x
     gll_x = @(s)  3*diag((s1-b1).^2) - diag(s2.^2); %segunda derivada de u1 em relação a x

     gl_y = @(s) (s2-b2).^3 - s2.*(s1.^2); %primeira derivada de u2 em relação a y
     gll_y = @(s) 3*diag((s2-b2).^2) - diag(s1.^2); %segunda derivada de u2 em relação a y
